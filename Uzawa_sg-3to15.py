...
def gamma(h,k,l,E):
    gamma_list =[]
    for i in range(0, len(h)):
        gamma_list.append((h[i]**4)*E[0] + (k[i]**4)*E[1] + (l[i]**4)*E[2] + 2*(h[i]**2)*(k[i]**2)*E[3] + 2*(l[i]**2)*(k[i]**2)*E[4] + 2*(h[i]**2)*(l[i]**2)*E[5] + 4*(h[i]**3)*(k[i])*E[6] + 4*(h[i])*(k[i]**3)*E[7] +\
        4*(k[i])*(h[i])*(l[i]**2)*E[8])
    return numpy.asarray(gamma_list)
# ----------------------------------#
def C_elements(h, k, l):
    return [h ** 4, k ** 4, l ** 4, 2 * (h ** 2) * (k ** 2), 2 * (l ** 2) * (k ** 2), 2 * (h ** 2) * (l ** 2),
            4 * (h ** 3) * (k), 4 * (k ** 3) * (h), 4 * h * k * (l ** 2)]
